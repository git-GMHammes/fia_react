...
$parametros_backend = array(
    ...
    'base_url' => base_url(),
    'api_get_profissao' => 'index.php/fia/ptpa/profissao/api/limpar',
    'api_post_filter_profissoes' => 'index.php/fia/ptpa/profissao/api/filtrarlixo',
);
$parametros_backend['base_paginator'] = implode('/', $parametros_backend['getURI']);
?>

<div class="app_limpar_profissao" data-result='<?php echo json_encode($parametros_backend); ?>'></div>

<script type="text/babel">
    const AppLimparProfissao = () => {

        // Variáveis recebidas do Backend
        const parametros = JSON.parse(document.querySelector('.app_limpar_profissao').getAttribute('data-result'));
            ...
            <div>
                ...
                <div className="table-responsive ms-2 me-2 ps-2 pe-2">
                        ...
                        <thead className="border border-2 border-dark border-start-0 border-end-0">
                            <tr>
                                ...
                                <th>
                                    <div className="collapse mb-4" style={formGroupStyle} id="collapseExample">
                                        <form action="">
                                            <input data-api="filtro-profissao" type="text" name="Restaurar" className="form-control form-control-sm" style={formControlStyle} placeholder="Restaurar" aria-label=".form-control-sm example" />
                                        </form>
                                    </div>
                                    RESTAURAR
                                </th>
                        ...
                        <tbody>
                            {profissoes.map((profissoes_value, index) => (
                                <tr key={index} className="col-12 col-sm-3 mb-5">
                                    ...
                                    <td>
                                        <a className="btn btn-outline-success" href={`${base_url}index.php/fia/ptpa/profissao/api/deletar/${profissoes_value.id}/restaurar${getVar_page}`} role="button">
                                            <i className="bi bi-reply-all"></i>
                                        </a>
                                    </td>
                                    <td>
                                        {/* Button trigger modal */}
                                        <button type="button" className="btn btn-outline-danger" data-bs-toggle="modal" data-bs-target={`#EliminarProfissaoModal${index}`}>
                    ...
                    {profissoes.map((profissoes_value, index) => (
                        <div key={index}>
                            {/* Modal */}
                            <div className="modal fade" id={`EliminarProfissaoModal${index}`} tabIndex="-1" aria-labelledby={`EliminarProfissaoModalLabel${index}`} aria-hidden="true">
                                <div className="modal-dialog">
                                    <div className="modal-content">
                                        <div className="modal-header">
                                            <h5 className="modal-title" id={`EliminarProfissaoModalLabel${index}`}>Confirmar a ELIMINAÇÃO permanente do Registro</h5>
                                        ...
                                        <div className="modal-footer">
                                            <a className="btn btn-outline-danger" href={`${base_url}index.php/fia/ptpa/profissao/api/deletar/${profissoes_value.id}/eliminar${getVar_page}`} role="button">
                                                Eliminar
                                            </a>
    ...
    //ReactDOM.render(<AppLimparProfissao />, document.querySelector('.app_limpar_profissao'));

    const rootElement = document.querySelector('.app_limpar_profissao');
    const root = ReactDOM.createRoot(rootElement);
    root.render(<AppLimparProfissao />);
