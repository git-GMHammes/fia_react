...
$parametros_backend['api_get_atualizar_profissao'] = ($atualizar_id !== 'erro') ? ('fia/ptpa/profissao/api/exibir' . $atualizar_id) : ('fia/ptpa/profissao/api/exibir/erro');
$parametros_backend['base_paginator'] = implode('/', $parametros_backend['getURI']);
?>

<div class="app_form_profissao" data-result='<?php echo json_encode($parametros_backend); ?>'></div>

<script type="text/babel">
    const AppFormProfissao = () => {

        // Variáveis recebidas do Backend
        const parametros = JSON.parse(document.querySelector('.app_form_profissao').getAttribute('data-result'));
        ...
        //Base Cadastro Profissao
        const api_get_atualizar_profissao = parametros.api_get_atualizar_profissao;
        ...
        const submitAllForms = (apiIdentifier) => {
            ...
            if (apiIdentifier == 'form-profissao') {
                console.log('form-profissao');
                ...
                fetch(`${base_url}${api_post_filter_profissoes}`, {
                    ...
                    .then(data => {
                        if (data.result.dbResponse && data.result.dbResponse.length > 0) {
                            // console.log('form-profissao:', data.result.dbResponse);
        ...
        // Fetch para obter os dados das Profissões
        console.log('React.useEffect: ');

        const fetchProfissoes = async () => {
            try {
                const response = await fetch(base_url + api_get_atualizar_profissao);
        ...
        return (
            <div>
                ...
                <form action={base_url + 'index.php/fia/ptpa/profissao/api/cadastrar'} method="post" className="row needs-validation m-2">
                                    ...
                                    <div className="row">
                                        <div className="col-12 col-sm-4 mb-3">
                                            <div style={formGroupStyle}>
                                                <label htmlFor="Codigo" style={formLabelStyle} className="form-label">Código da Profissão</label>
    ...
    //ReactDOM.render(<AppFormProfissao />, document.querySelector('.app_form_profissao'));

    const rootElement = document.querySelector('.app_form_profissao');
    const root = ReactDOM.createRoot(rootElement);
    root.render(<AppFormProfissao />);
</script>
<?php
$parametros_backend = array();
?>