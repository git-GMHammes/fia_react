...
$parametros_backend = array(
    ...
    'base_url' => base_url(),
    'api_get_genero' => 'index.php/fia/ptpa/genero/api/limpar',
    'api_post_filter_generos' => 'index.php/fia/ptpa/genero/api/filtrarlixo',
);
$parametros_backend['base_paginator'] = implode('/', $parametros_backend['getURI']);
?>

<div class="app_limpar_generoidentidade" data-result='<?php echo json_encode($parametros_backend); ?>'></div>

<script type="text/babel">
    const AppLimparGeneroIdentidade = () => {
        
        // Variáveis recebidas do Backend
        const parametros = JSON.parse(document.querySelector('.app_limpar_generoidentidade').getAttribute('data-result'));
       ...
       const submitAllForms = (apiIdentifier) => {
            ...
            if(apiIdentifier == 'filtro-genero'){
                ...
                // fetch(`${base_url}${api_post_filter_generos}/${apiIdentifier}${getVar_page}`, {
            ...
            <div>
                ...
                <div className="table-responsive ms-2 me-2 ps-2 pe-2">
                        ...
                        <tbody>
                            {generos.map((generos_value, index) => (
                                <tr key={index}>
                                    ...
                                    <td>
                                        <a className="btn btn-outline-success" href={`${base_url}index.php/fia/ptpa/genero/api/deletar/${generos_value.id}/restaurar${getVar_page}`} role="button">
                                            <i className="bi bi-reply-all"></i>
                                        </a>
                                    </td>
                                    <td>
                                            {/* Button trigger modal */}
                                        <button type="button" className="btn btn-outline-danger" data-bs-toggle="modal" data-bs-target={`#EliminarGeneroIdentidadeModal${index}`}>
                    ...
                    {generos.map((generos_value, index) => (
                        <div key={index}>
                            {/* Modal */}
                            <div className="modal fade" id={`EliminarGeneroIdentidadeModal${index}`} tabIndex="-1" aria-labelledby={`EliminarGeneroIdentidadeModalLabel${index}`} aria-hidden="true">
                                <div className="modal-dialog">
                                    <div className="modal-content">
                                        <div className="modal-header">
                                            <h5 className="modal-title" id={`EliminarGeneroIdentidadeModalLabel${index}`}>Confirmar a ELIMINAÇÃO permanente do Registro</h5>
                                            <button type="button" className="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                        </div>
                                        <div className="modal-body">
                                            <div className="d-flex justify-content-center">
                                               <b>Genero Identidade:&nbsp;</b>{generos_value.genero}<br/>
                                            </div>
                                            <hr/>
                                            <div className="d-flex justify-content-center">
                                        ...
                                        <div className="modal-footer">
                                            <a className="btn btn-outline-danger" href={`${base_url}index.php/fia/ptpa/genero/api/deletar/${generos_value.id}/eliminar${getVar_page}`} role="button">
                                                Eliminar
                                            </a>
    ...
    //ReactDOM.render(<AppLimparGeneroIdentidade />, document.querySelector('.app_limpar_generoidentidade'));

    const rootElement = document.querySelector('.app_limpar_generoidentidade');
    const root = ReactDOM.createRoot(rootElement);
    root.render(<AppLimparGeneroIdentidade />);
</script>
<?php
$parametros_backend = array();
?>