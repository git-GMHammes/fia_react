...
$parametros_backend = array(
    ...
    'api_get_responsavel' => 'index.php/fia/ptpa/responsavel/api/excluir',
    'api_post_filter_responsaveis' => 'index.php/fia/ptpa/responsavel/api/filtrar',
);
?>

<div class="app_excluir_responsavel" data-result='<?php echo json_encode($parametros_backend); ?>'></div>

<script type="text/babel">
    const AppExcluirResponsavel = () => {
        // Variáveis recebidas do Backend
        const parametros = JSON.parse(document.querySelector('.app_excluir_responsavel').getAttribute('data-result'));
            ...
            <div>
                ...
                <div className="table-responsive ms-2 me-2 ps-2 pe-2">
                    <table className="table table-hover">
                        <thead>
                            <tr>
                                ...
                                <th scope="col" className="text-nowrap">
                                    <div className="collapse mb-4" style={formGroupStyle} id="collapseExample">
                                        <button className="btn" onClick={() => submitAllForms('filtro-resposavel')} type="submit">Excluir</button>
                                    </div>
                                    EXCLUIR
                                </th>
                            </tr>
                    ...
                    {responsaveis.map((responsaveis_value, index) => (
                        <div key={index}>
                            {/* Modal */}
                            <div className="modal fade" id={`ExcluirResponsavelModal${index}`} tabIndex="-1" aria-labelledby={`ExcluirResponsavelModalLabel${index}`} aria-hidden="true">
                                <div className="modal-dialog">
                                    <div className="modal-content">
                                        <div className="modal-header">
                                            <h5 className="modal-title" id={`ExcluirResponsavelModalLabel${index}`}>Confirmar a EXCLUSÃO permanente do Registro</h5>
                                        ...
                                        <div className="modal-footer">
                                            <a className="btn btn-outline-danger" href={`${base_url}index.php/fia/ptpa/responsavel/api/deletar/${responsaveis_value.id}${getVar_page}`} role="button">
                                                Excluir
                                            </a>
                    ...
                    <nav aria-label="Responsaveis">
                        <ul className="pagination">
                            {pagination === true && paginacaoResponsaveis.map((paginacao_responsaveis_value, index) => {
                                ...
                                return (
                                    <li
                                        key={index} className={`page-item ${isActive ? 'active' : ''} ${paginacao_responsaveis_value.disabled ? 'disabled' : ''}`}>
                                        <a
                                            className="page-link"
    ...
    //ReactDOM.render(<AppExcluirResponsavel />, document.querySelector('.app_excluir_responsavel'));

    const rootElement = document.querySelector('.app_excluir_responsavel');
    const root = ReactDOM.createRoot(rootElement);
    root.render(<AppExcluirResponsavel />);
</script>