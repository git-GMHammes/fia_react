...
<div class="app_excluir_programa" data-result='<?php echo json_encode($parametros_backend); ?>'></div>

<script type="text/babel">
    const AppExcluirPrograma = () => {

        // Variáveis recebidas do Backend
        const parametros = JSON.parse(document.querySelector('.app_excluir_programa').getAttribute('data-result'));
                ...
                <div className="table-responsive ms-2 me-2 ps-2 pe-2">
                        ...
                        <thead>
                            <tr>
                                ...
                                <th scope="col" className="text-nowrap">
                                    <div className="collapse" style={formGroupStyle} id="collapseExample">
                                        <form action="">
                                            <input data-api="filtro-programa" className="form-control form-control-sm" style={formControlStyle} type="text" name="Descricao" placeholder="Descricao" aria-label=".form-control-sm example" />
                                        </form>
                                    </div>
                                    DESCRIÇÃO
                                </th>
                                <th scope="col" className="text-nowrap">
                                    <div className="collapse" style={formGroupStyle} id="collapseExample">
                                        <form action="">
                                            <input data-api="filtro-programa" className="form-control form-control-sm" style={formControlStyle} type="text" name="Link" placeholder="Link" aria-label=".form-control-sm example" />
                                        </form>
                                    </div>
                                    LINK
                                </th>
                                <th scope="col" className="text-nowrap">
                                    <div className="collapse" style={formGroupStyle} id="collapseExample">
                                        <button className="btn" onClick={() => submitAllForms('filtro-programa')} type="submit">Filtrar</button>
                                    </div>
                                    EXCLUIR
                                </th>
                            </tr>
                                    ...
                                    <div className="modal-content">
                                        ...
                                        <div className="modal-body">
                                            <div className="d-flex justify-content-center">
                                                <b>Sigla:&nbsp;</b>{programas_value.Sigla}<br />
                                            </div>
                                            <hr />
                                            <div className="d-flex justify-content-center">
                                                <b>Descrição:&nbsp;</b>{programas_value.Descricao}<br />
                                            </div>
                                            <hr />
                                        </div>
                                        <div className="modal-footer">
                                            <a className="btn btn-outline-danger" href={`${base_url}index.php/fia/ptpa/programa/api/deletar/${programas_value.id}${getVar_page}`} role="button">
                                                Excluir
                                            </a>
                    ...
                    <nav aria-label="Programas">
                        <ul className="pagination">
                            {pagination === true && paginacaoProgramas.map((paginacao_programas_value, index) => {
                                ...
                                return (
                                    <li
                                        key={index}
                                        className={`page-item ${isActive ? 'active' : ''} ${paginacao_programas_value.disabled ? 'disabled' : ''}`}
                                    >
                                        <a
                                            className="page-link"
    ...
    //ReactDOM.render(<AppExcluirPrograma />, document.querySelector('.app_excluir_programa'));

    const rootElement = document.querySelector('.app_excluir_programa');
    const root = ReactDOM.createRoot(rootElement);
    root.render(<AppExcluirPrograma />);
