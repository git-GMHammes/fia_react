<script type="text/babel">
    const AppNomeProfissional = (
        ...
        const api_get_profissional = parametros.api_get_profissional || 'erro';
        ...
        const handleChange = (event) => {
            ...
            // Verifica se a mudança é no campo 'profissional_nome'
            if (name === 'profissional_nome') {
                console.log('profissional_nome :: ', value);
            ...
            if (name === 'pesquisa_nome_ado') {
                ...
                setFormData((prev) => ({
                    ...prev,
                    profissional_nome: pesquisa_nome_ado
                }));
                ...
                debounceTimeout.current = setTimeout(() => {
                    console.log('fetchFiltrarProfissional');
                    fetchFiltrarProfissional();
                }, 100);
            }
        ...
        // POST fetchProfissional 
        const fetchProfissional = async (custonBaseURL = base_url, custonApiPostProfissional = api_get_profissional, customPage = getVar_page) => {
            const url = custonBaseURL + custonApiPostProfissional + customPage + "&limit=90000";
            console.log('fetchProfissional :: ', url);
            const setData = { 'PerfilId': '5'};
            try {
                ...
                // console.log('data(fetchProfissional) :: ', data);
                if (data.result && data.result.dbResponse && data.result.dbResponse.length > 0) {
                    ...
                    setMessage({
                        show: true,
                        type: 'light',
                        message: 'Não foram encontrados Profissionais cadastrados'
                    });
                }
            } catch (error) {
        ...
        // POST pesquisa Profissionais
        const fetchFiltrarProfissional = async (custonBaseURL = base_url, custonApiPostObjeto = api_get_profissional, customPage = getVar_page) => {
        ...
        return (
            <div>
                <div style={formGroupStyle}>
                    ...
                    >
                        Profissional<strong style={requiredField}>*</strong>
                    </label>
                    {(isLoading) ? (
                        <div>
                            <div>&nbsp;</div>
                        ...
                        <div className="btn-group w-100">
                            ...
                            >
                                <span>
                                    {typeof formData.profissional_nome === 'string' ? formData.profissional_nome : 'Seleção Nula'}
                                </span>
                                <i className="bi bi-chevron-down"></i>
                            </button>
                            <div className="dropdown-menu w-100 overflow-auto" style={{ height: '380px' }}>
                                <input
                                    data-api={`filtro-filtro-profissional`}
                                    type="text"
                                ...
                                <input
                                    ...
                                    onBlur={handleBlur}
                                />
                                {profissionais.map((profissional_select) => (
                                    <div key={profissional_select.id}>
                                        <input
                                            data-api={`filtro-${origemForm}`}
                                            type="radio"
                                            style={formControlStyle}
                                            ...
                                            id={`profissional-${profissional_select.id}`}
                                            value={`${profissional_select.id}/${profissional_select.Nome}`}
                                            onFocus={handleFocus}
                                            onChange={handleChange}
                                            onBlur={handleBlur}
                                            autoComplete="off"
                                        ...
                                        <label
                                            ...
                                            htmlFor={`profissional-${profissional_select.id}`}
                                        >
                                            {profissional_select.Nome}
                                        </label>
                                    </div>
                                ))}
                            </div>
